...
export default async function handler(req, res) {
  ...
  if (action === 'doc-track-view' && req.method === 'POST') {
    ...
    const { data: doc } = await db.from('documents').select('id,name,status,viewed_at,sent_by,contact_email').eq('sign_token', tok).single();
  ...
  if (action === 'doc-sign' && req.method === 'POST') {
    ...
    const { data: doc } = await db.from('documents').select('id,name,status,sign_token,sent_by,contact_id,company_id,html_content,contact_email').eq('sign_token', tok).single();
    if (!doc) return res.status(404).json({ error: 'Document not found' });
